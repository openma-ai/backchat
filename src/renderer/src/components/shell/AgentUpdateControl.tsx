  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent
        ...
        className="gap-0 overflow-hidden p-0 sm:max-w-[380px]"
      >
        <DialogHeader className="gap-1 border-b border-border px-4 py-3.5 pr-12">
          <div className="flex items-start justify-between gap-4">
            ...
            {availableAgents.length > 0 && (
              <span className="mt-0.5 shrink-0 text-[11px] font-medium tabular-nums text-warning">
        ...
        {recentlyUpdated && (
          <div
            role="status"
            className="flex items-center gap-2 border-b border-success/20 bg-success-subtle/65 px-4 py-2.5 text-xs font-medium text-success"
          >
        ...
        <div className="max-h-[min(360px,calc(100vh-160px))] overflow-y-auto">
          {availableAgents.map((agent) => {
            ...
            return (
              <div
                key={agent.id}
                className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 border-b border-border/70 px-4 py-3.5 last:border-b-0"
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-fg">{agent.label}</p>
                  <p className="mt-0.5 text-xs tabular-nums text-fg-muted">
                    {updateDescription(agent, t("acpUpdates.newerRuntime"))}
                  </p>
                  {error && (
                    <p className="mt-1.5 flex items-start gap-1.5 text-xs leading-5 text-danger">
                      <TriangleAlertIcon
                ...
                <Button
                  ...
                  className="mt-0.5 h-8 min-h-0 gap-1.5 px-2.5 text-xs"
                >
                  {updating && (
                    <Loader2Icon className="size-3.5 animate-spin" aria-hidden="true" />
                  )}
        ...
        <p className="border-t border-border bg-bg-surface/55 px-4 py-2.5 text-[11px] leading-4 text-fg-muted">
          {t("acpUpdates.runningNotice")}
        </p>
      </DialogContent>
    </Dialog>
  );
