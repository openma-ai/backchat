...
describe("resuming a session restores config the agent dropped", () => {
  const collaboration = (currentValue: string): AcpSessionConfigOption => ({
  ...
  const resumeWith = (
    ...
    store.apply({
      ...
      cwd: "/tmp/resume",
      acp_session_id: "acp-1",
      config_options: [collaboration(remembered)],
    } as never);
    // Second readiness is the resume after a restart: a fresh agent process
    // reports whatever state it managed to restore.
    store.apply({
      type: "session.ready",
      session_id: "sess-resume",
      agent_id: "codex-acp",
      cwd: "/tmp/resume",
      acp_session_id: "acp-1",
      config_options: [collaboration(reported)],
    } as never);
    return sessionSetConfigOption;
  };

  test("pushes plan mode back when a restarted agent reports default", () => {
    // ACP v1 says a resume response MAY carry configuration state; codex-acp
    // keeps the collaboration mode in a process-local map, so plan silently
    // becomes an editable session after a restart.
    const setConfigOption = resumeWith("plan", "default");
  ...
  test("stays quiet when the agent restored the value itself", () => {
    expect(resumeWith("plan", "plan")).not.toHaveBeenCalled();
  });
