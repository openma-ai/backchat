...
export interface SessionManagerDeps {
   ...
   *  user's globally-configured servers minus any per-agent disables. Phase 8
   *  wires this to McpConfigStore; until then a stub returning `[]` keeps the
   *  manager local-only. */
...
export class SessionManager {
  ...
  async start(p: SessionStartParams): Promise<void> {
    ...
    const onPath = await new Promise<boolean>((resolve) => {
      ...
      const proc = childSpawn(probe, [agent.spec.command], { stdio: "ignore" });
    ...
    const sessionCwd = p.cwd ?? (await ensureSessionCwd(p.session_id));
