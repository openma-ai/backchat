...
it.each(["claude-managed", "openai-agents"] as const)("establishes a delayed %s stream before history catch-up", async (provider) => {
  const client = new runtime.DirectAgentRuntime({ ...options, provider, fetchImpl: async (_input, init) => {
  ...
  let connected = false;
  const events = [];
  for await (const event of client.stream("s", { onConnected: () => { connected = true; } })) events.push(event);
  expect(connected).toBe(true);
  expect(events).toEqual([]);
});

it("passes the desktop operation identity to the OpenAI SDK without event metadata", async () => {
  let sent: Headers | undefined;
  const client = new runtime.DirectAgentRuntime({ ...options, provider: "openai-agents", fetchImpl: async (_input, init) => {
    sent = new Headers(init?.headers);
    return new Response(null, { status: 204 });
  } });
  await client.sendEvent("s", { type: "user.message", content: [{ type: "text", text: "once" }] }, "desktop-op");
  expect(sent?.get("Idempotency-Key")).toBe("desktop-op");
});
