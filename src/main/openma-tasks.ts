...
export class OpenmaTasks {
  ...
  async #observe(id: string, observer: Observer) {
    ...
    while (!signal.aborted) {
      let live: Awaited<ReturnType<DirectAgentRuntime["openStream"]>> | undefined;
      try {
        const task = this.#task(id);
        ...
        if (client instanceof DirectAgentRuntime) live = await client.openStream(task.sessionId, signal);
        ...
        const session = await client.retrieveSession(task.sessionId, signal);
        if (signal.aborted) return;
        ...
        for await (const event of live?.events ?? client.stream(task.sessionId, { afterSeq: this.#store.get(id)!.afterSeq, signal })) {
          if (signal.aborted) return;
          this.#ingest(id, observer, event);
