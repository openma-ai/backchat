Create Codex pet row `running-right` for `mote`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact friendly desk-spark companion for a standalone desktop pet app: rounded translucent body, warm vermilion accent, small curious face, helpful but independent personality, no logos, no text.. Preserve silhouette, face, palette, material, proportions, markings, and props.
